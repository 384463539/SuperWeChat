...
public class GroupDetailsActivity extends BaseActivity implements OnClickListener {
    ...
    private void loadMyGroup() {
        NetDao.findGroupXX(this, groupId, new OkHttpUtils.OnCompleteListener<Result>() {
            @Override
            public void onSuccess(Result result) {
                if (result != null && result.isRetMsg()) {
                    ...
                    mygroupId = groupBean.getMGroupId().toString();
                }
            }
        ...
        if (resultCode == RESULT_OK) {
            ...
            switch (requestCode) {
                case REQUEST_CODE_ADD_USER:// 添加群成员
                    ...
                    progressDialog.setMessage(st1);
                    progressDialog.show();
                    addAppMembersToGroup(newmembers);
                    break;
                ...
                case REQUEST_CODE_EDIT_GROUPNAME: //修改群名称
                    ...
                    if (!TextUtils.isEmpty(returnData)) {
                        ...
                        progressDialog.show();
                        NetDao.updateGroupName(this, returnData, mygroupId, new OkHttpUtils.OnCompleteListener<Result>() {
                            @Override
                            public void onSuccess(Result result) {
                                if (result != null && result.isRetMsg()) {
                                    Toast.makeText(getApplicationContext(), "更新群名称成功", Toast.LENGTH_SHORT).show();
                                    progressDialog.dismiss();
                                } else {
                                    progressDialog.dismiss();
                                    Toast.makeText(getApplicationContext(), "更新群名称失败", Toast.LENGTH_SHORT).show();
                                }
                            }
    ...
    private void exitAppGroup() {
        NetDao.deletMember(this, EMClient.getInstance().getCurrentUser(), mygroupId, new OkHttpUtils.OnCompleteListener<Result>() {
            @Override
            public void onSuccess(Result result) {
                if (result != null && result.isRetMsg()) {
                    exitGrop();
                    Toast.makeText(getApplicationContext(), "退群成功", Toast.LENGTH_SHORT).show();
                } else {
                    progressDialog.dismiss();
                    Toast.makeText(getApplicationContext(), "退群失败", Toast.LENGTH_SHORT).show();
                }
            }
    ...
    private void deleteAppGrop() {
        NetDao.deleteGroup(this, mygroupId, new OkHttpUtils.OnCompleteListener<Result>() {
            @Override
            public void onSuccess(Result result) {
                if (result != null && result.isRetMsg()) {
    ...
    //增加群成员//
    private void addAppMembersToGroup(final String[] newmembers) {
        NetDao.addMembers(this, newmembers, groupId, new OkHttpUtils.OnCompleteListener<Result>() {
            @Override
            public void onSuccess(Result result) {
                if (result != null && result.isRetMsg()) {
...
//				button.setCompoundDrawables(null, avatar, null, null);
                EaseUserUtils.setUserNick2(username, holder.textView);
                EaseUserUtils.setUserAvatar2(getContext(), username, holder.imageView);
                if (isInDeleteMode) {
                    // 如果是删除模式下，显示减人图标
                ...
                button.setOnClickListener(new OnClickListener() {
                    ...
                    protected void deleteAppMembersFromGroup(final String username) {
                        NetDao.deletMember(mcontext, username, mygroupId, new OkHttpUtils.OnCompleteListener<Result>() {
                            @Override
                            public void onSuccess(Result result) {
                                if (result != null && result.isRetMsg()) {
                                    deleteMembersFromGroup(username);
